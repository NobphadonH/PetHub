...
function HomeHotelBox({
  ...
  const goHotelDetail = () => {
    if (checkIn && checkOut) {
      ...
      naigate(`./pethub-website/home/${hotelName}`, { state: hotelData });
    } else {
      ...
      <div className="text-start flex grow flex-col justify-between max-md:gap-0 max-lg:gap-5">
          ...
          <div className="flex lg:my-5 gap-1 items-center">
            {Array.from({ length: rating }).map((_, index) => (
              <svg
                ...
                height="16"
                fill="currentColor"
                className="bi bi-star-fill max-md:w-[6px] text-yellow-400"
                viewBox="0 0 16 16"
              >
        ...
        <div className="p-2 md:px-3 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <button
              onClick={goHotelDetail}
              className="flex justify-center items-center rounded-md md:btn bg-pethub-color1 md:bg-pethub-color1 text-white md:text-white w-[17vw] max-md:text-[2vw] h-[7vw] md:w-32 font-medium"
            >
              รายละเอียด
            </button>
          ...
          <div className="flex gap-3 items-center">
            <svg
              ...
              height="24"
              fill="currentColor"
              className="bi bi-cash hidden lg:block"
              viewBox="0 0 16 16"
            >
            ...
            <span className="text-[2.5vw] md:text-xl">
              {price}{" "}
              <span className="max-md:text-[1.5vw] text-sm">บาท / คืน</span>
            </span>
          </div>
        </div>
      </div>
...
HomeHotelBox.defaultProps = {
  ...
  rating: 3,
  description:
    "Lorem ipsum dolor sit amet consectetur adipisicing elit. Nisi, possimus perferendis voluptatem distinctio laboriosam harum ullam ea, quam aperiam quos numquam, deleniti mollitia placeat rerum corporis. Architecto id necessitatibus adipisci!",
  price: "N/A",
  link: "/",
  imageUrl: "",
  petType: ["🐱", "🐶"],
