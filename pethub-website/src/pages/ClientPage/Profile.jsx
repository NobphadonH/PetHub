...
function Profile() {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      const response = await axios.get(
        `https://pethub-3nkk.onrender.com/api/user/getProfilebyUserID`,
        {
  ...
  const fetchPets = async () => {
    try {
      setLoading(true);
      const response = await axios.get(
        `https://pethub-3nkk.onrender.com/api/pet/getAllPetsByUserID`,
        {
          withCredentials: true,
        }
      );
  ...
  const fetchbookingStatus = async () => {
    try {
      setLoading(true);
      const response = await axios.get(
        `https://pethub-3nkk.onrender.com/api/booking/getBookingStatusbyUserID`,
        {
          withCredentials: true,
        }
      );
  ...
  const handleCancelBooking = async (bookingID) => {
    try {
      const response = await axios.post(
        "https://pethub-3nkk.onrender.com/api/booking/cancelBooking",
        { bookingID },
        {
          withCredentials: true,
        }
  ...
  useEffect(() => {
    ...
    if (!role) {
      naigate(`./pethub-website/signin`);
    } else if (role === "Client") {
      naigate(`./pethub-website/profile`);
    } else if (role === "Host") {
      naigate(`./pethub-website/hostprofile`);
    }
  }, [Cookies]);
    ...
    <>
      ...
      <div className="text-start mt-20 lg:mt-32 bg-pethub-color1 w-11/12 xl:w-10/12 max-w-[1200px] h-[30vw] sm:h-[200px] md:h-44 mx-auto p-8 md:py-10 md:px-12 xl:px-28 relative rounded-md z-10 shadow-md max-lg:overflow-hidden">
        ...
        <div className="max-lg:hidden absolute top-32 right-8 left-8 h-[330px] xl:h-[360px] flex justify-between gap-16">
          <div className="w-[250px] h-full bg-white shadow-md flex flex-col p-3 rounded-md">
            ...
            <div className="text-center text-xl mt-5">{`คุณ ${
              user.fName || "ชื่อผู้ใช้"
            } ${user.lName || ""}`}</div>
          </div>
          <div className=" lg:w-[700px] xl:w-[780px] pt-20">
            <div className="grid grid-cols-12 gap-x-10">
              <div className="col-span-6">
                <p className="text-start text-[2.5vw] md:text-xs lg:text-sm xl:text-sm my-[1vw] md:my-3">
                  เบอร์โทร
                </p>
                <input
                  type="text"
              ...
              <div className="col-span-12 flex justify-end xl:mt-10">
                <div className="flex justify-center items-center rounded-md md:btn bg-pethub-color6 md:bg-pethub-color6 text-white md:text-white h-[7vw] w-[15vw] sm:w-36 sm:h-10 md:w-28 font-medium text-[2vw] md:text-xs lg:text-sm xl:text-base">
                  แก้ไขข้อมูล
                </div>
              </div>
            </div>
      ...
      <div className="lg:hidden w-11/12 xl:w-10/12 max-w-[1200px] h-[40vw] md:h-72 mx-auto mt-8 md:px-10">
        <div className="flex max-md:gap-[8vw] md:justify-between w-full h-full">
          <div className="w-[30vw] md:w-[220px] h-full shadow-md flex flex-col p-[1vw]">
            ...
            <div className="text-center text-[2.5vw] md:text-xs lg:text-sm xl:text-sm mt-[3vw] md:my-3">{`คุณ ${
              user.fName || "ชื่อผู้ใช้"
            } ${user.lName || ""}`}</div>
          </div>
      ...
      <div className="w-11/12 xl:w-10/12 mx-auto mb-24 lg:mb-32 mt-10 lg:mt-[400px] max-w-[1200px]">
        <div className="w-full flex justify-between items-center">
          <div className="text-[3vw] md:text-lg lg:text-2xl xl:text-3xl font-bold text-pethub-color6">
            สัตว์เลี้ยง<span className="text-pethub-color1">ของฉัน</span>
          </div>
        </div>
        {petData.length === 0 ? (
              ...
              <div className="w-full h-[26vw] md:h-52 lg:h-60 xl:h-72 rounded-md p-[1vw] md:p-3 border-[1px] flex gap-[1vw] md:gap-5">
                ...
                <div className="w-[600px] lg:grow flex flex-col p-[1vw] md:p-3 lg:p-5 pb-0 justify-between">
                  <div className="flex flex-wrap justify-between text-[2vw] md:text-base lx:text-lg font-semibold text-pethub-color6">
                    <div>
                      ชื่อ:{" "}
                      <span className="text-pethub-color1">{pet.petName}</span>
                    </div>
                    <div>
                      อายุ:
                    ...
                    <div>
                      ประเภท:{" "}
                      <span className="text-pethub-color1">{pet.petType}</span>
                    </div>
                    <div>
                      เพศ:{" "}
                      <span className="text-pethub-color1">{pet.petSex}</span>
                    </div>
                  </div>
                  <div>
                    <p className="text-start font-semib text-[2vw] md:text-xs xl:text-sm my-1 lg:my-3">
                      คำอธิบายลักษณะเพิ่มเติม
                    </p>
        ...
        <div className="w-full flex justify-end">
          <a
            href="/pethub-website/petregister"
            className="flex justify-center items-center rounded-md md:btn bg-pethub-color1 md:bg-pethub-color1 text-white md:text-white h-[7vw] w-[15vw] sm:w-36 sm:h-10 md:w-28 lg:w-32 font-medium text-[2vw] md:text-xs lg:text-sm xl:text-base"
          >
            เพิ่มสัตว์เลี้ยง
          </a>
        </div>
      ...
      <div className="w-11/12 xl:w-10/12 max-w-[1200px] mx-auto lg:px-8 -my-16 lg:my-24">
        <div className="w-full text-start text-[3vw] md:text-lg lg:text-2xl xl:text-3xl font-bold text-pethub-color6">
          ประวัติ<span className="text-pethub-color1">การจองของฉัน</span>
        </div>
                ...
                <tbody key={index}>
                  <tr className="text-xs lg:text-sm xl:text-base">
                    ...
                    <td className="max-md:pr-0">
                      <span
                        className={`badge badge-ghost badge-sm text-[2.5vw] sm:text-xs lg:text-sm xl:text-base ${
                    ...
                    <th>
                      ...
                      >
                        <span className="max-lg:hidden ">รายละเอียด</span>
                        <span>
                          <svg
                  ...
                  {/* detail */}
                  <tr
                    className={`${isClick[index] ? "" : "hidden"} bg-slate-50`}
                  >
                    <td colSpan={6} className="text-sm">
                      ...
                      <div className="flex gap-[3vw] md:gap-10 my-[1vw] md:my-3 font-semibold ">
                        <div className="badge badge-ghost badge-md font-normal text-[2.5vw] md:text-xs lg:text-sm">
                          การจอง: <span>2 คืน</span>
                        </div>
                        <div className="badge badge-ghost badge-md font-normal text-[2.5vw] md:text-xs lg:text-sm">
                          เช็คอิน:{" "}
                          <span className="font-normal">
                      ...
                      <div className="mt-[2vw] text-[3vw] md:text-sm md:mt-6 font-semibold">
                        ข้อมูลสัตว์เลี้ยงที่เข้าพัก
                      </div>
                      <div
                        className={`bg-white w-full relative h-28 sm:h-36 lg:h-40 xl:h-44 border-[1px] my-[2vw] md:my-3 rounded-md p-1 flex overflow-y-scroll hide-scrollbar`}
                      >
                        ...
                        <div className="w-full overflow-y-hide hide-scrollbar h-full px-2 py-[2vw] md:py-3 xl:p-5 flex flex-col justify-between">
                          ...
                          <p className="text-start text-[2vw] md:text-[1.2vw] xl:text-sm my-1 lg:my-3">
                            คำอธิบายลักษณะเพิ่มเติม
                          </p>
                          <textarea
                            className="textarea w-full max-md:p-[1vw] max-h-8 min-h-8 md:max-h-12 md:min-h-12 lg:min-h-16 lg:max-h-16 textarea-bordered hide-scrollbar text-[1.5vw] md:text-[1vw] xl:text-sm text-gray-600"
                            value={booking.petDetail}
                          ></textarea>
                        </div>
                      </div>
                      <div className="my-[2vw] md:my-4 w-full flex justify-between gap-5 items-center">
                        <div className="text-[2vw] md:text-xs lg:text-sm">
                          ติดต่อสอบถาม: {booking.hotelName}, 094-XXX-XXXX
                        </div>
                        <div
                          ...
                          className={`${
                            booking.bookingStatus === "Cancelled"
                              ? "hidden"
                              : "flex justify-center items-center rounded-md md:btn bg-red-600 md:bg-red-600 text-white md:text-white h-[7vw] w-[15vw] sm:w-36 sm:h-10 md:w-40 font-medium text-[2vw] md:text-xs lg:text-sm xl:text-base"
                          } `}
                        >
                          ยกเลิกการจอง
                        </div>
