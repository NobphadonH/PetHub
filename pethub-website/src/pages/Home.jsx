...
function Home() {
  ...
  const handleSearchClick = (e) => {
    ...
    setIsFetch(0);
    naigate(`./pethub-website/home?${query}`);
  };
  ...
  const handlePriceRangeChange = (e) => {
    const [lower, upper] = e.target.value
      .split("-")
    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          `https://pethub-3nkk.onrender.com/api/roomSearch/getHotelAndRoomByFilter/?${queryParams}`
        );
  ...
  useEffect(() => {
    ...
    const updatedWithPrices = updatedWithPetType.map((hotel) => {
      const lowestPrice = hotel.roomsAvailable.reduce(
        (minPrice, room) => Math.min(minPrice, room.pricePerNight),
        hotel.roomsAvailable[0]?.pricePerNight || Infinity
      );
      const highestPrice = hotel.roomsAvailable.reduce(
        (maxPrice, room) => Math.max(maxPrice, room.pricePerNight),
  ...
  return (
    <div>
      ...
      <div className="text-start mt-20 lg:mt-32 bg-pethub-color1 w-11/12 xl:w-10/12 max-w-[1200px] h-[250px] sm:h-[270px] md:h-full mx-auto p-8 md:py-10 md:px-12 xl:px-28 relative rounded-md overflow-hidden z-10 opacity-80">
        ...
        <h1 className="text-5xl md:text-7xl relative z-20 text-pethub-color1  ">
          PET HUB
        </h1>
        <h4 className="text-sm lg:text-lg relative z-20">
          เลือกโรงแรมให้เหมาะสมกับน้องๆของคุณ
        </h4>
        <div className="grid max-md:grid-rows-2 grid-cols-12 md:grid-cols-12 gap-5 lg:gap-7 xl:gap-14 mt-8 md:mt-24 relative z-20">
          <div className="hidden md:block col-span-3">
            ...
            >
              ...
              <option value="สุนัข" style={{ color: "black" }}>
                สุนัข
              </option>
              <option value="แมว" style={{ color: "black" }}>
                แมว
              </option>
              <option value="อื่น ๆ" style={{ color: "black" }}>
                สัตว์ประเภทอื่นๆ
              </option>
            </select>
          </div>
          <div className="hidden md:block col-span-4">
            <select
              ...
              onChange={handleInputChange}
              className="select select-bordered w-full max-w-xs input-shadow"
              style={{ color: "gray" }}
            >
              <option disabled selected style={{ color: "gray" }}>
                สถานที่ตั้ง
          ...
          <div className="hidden md:block col-span-3">
            ...
            >
              ...
              <option value="500-2000" style={{ color: "black" }}>
                500-2000 บาท
              </option>
              <option value="2000-5000" style={{ color: "black" }}>
                2000-5000 บาท
              </option>
            </select>
          </div>
          ...
          <div className="max-md:row-start-2 col-span-12 md:col-span-4">
            <label className="input input-bordered flex items-center gap-2 h-[10vw] max-h-12 max-w-full input-shadow  text-[3vw] sm:text-xs lg:text-base">
              <input
                ...
                onChange={handleInputChange}
                className="grow"
                placeholder="ค้นหาจากชื่อโรงแรม"
              />
              <a href="">
                <svg
          ...
          <div className="col-span-2">
            <button
              ...
              className="btn bg-pethub-color1 border-pethub-color1 font-medium max-md:hidden text-white relative z-20"
            >
              ค้นหา
            </button>
          </div>
      ...
      <div className="mx-auto my-5 grid grid-cols-12 w-11/12 md:w-[750px] h-full lg:w-full gap-5 lg:gap-10">
        ...
        <div className="md:hidden col-span-12 flex justify-center">
          <button
            onClick={handleSearchClick}
            className="flex justify-center items-center rounded-md md:btn bg-pethub-color1 md:bg-pethub-color1 text-white md:text-white max-sm:text-[2.5vw] h-[7vw] w-[20vw] sm:w-32 md:w-32 font-medium text-xs lg:text-sm xl:text-base"
          >
            ค้นหา
          </button>
        </div>
        ...
        <div className="col-span-12">
          <div className="my-2 lg:my-5 text-[3vw] md:text-sm lg:text-lg text-pethub-color1">
            {hotelResult.length} ผลการค้นหา
          </div>
        </div>
      ...
      <div className="w-11/12 xl:w-8/12 h-10 mx-auto flex justify-center items-center mt-8 md:mt-20 gap-5">
        <div
          className="max-md:text-[2.5vw] flex justify-center items-center cursor-pointer"
          onClick={() => handlePrev()}
        >
          Prev
        </div>
        {Array.from({ length: pagenumber }).map((page, index) => (
          <div
            ...
            className={
              index == pageselect
                ? `bg-pethub-color1 text-white w-8 h-8 md:w-10 md:h-10 flex items-center justify-center cursor-pointer`
                : `bg-base-200 w-8 h-8 md:w-10 md:h-10 flex items-center justify-center cursor-pointer`
            }
          >
            {index + 1}
          </div>
      ...
      <div className="mx-auto flex justify-between w-11/12 xl:w-8/12 mt-40 items-center">
        <div className="text-[4vw] md:text-3xl font-semibold text-pethub-color6">
          โรงแรมที่แนะนำ
        </div>

        <div className="w-[27vw] h-[10vw] max-h-10 text-[2vw] text-end sm:text-lg lg:hidden">
          <select
            className="select select-bordered w-full max-w-28 h-full border-2 px-3 rounded-full"
            style={{ color: "gray" }}
          >
            <option disabled selected style={{ color: "gray" }}>
              ประเภทสัตว์
        ...
        <div className="hidden lg:flex gap-5">
          <div className="btn rounded-full bg-pethub-color1 text-white">
            <span className="text-xl">🐱</span>แมว
          </div>
          <div className="btn rounded-full bg-slate-200">
            <span className="text-xl">🐶</span>สุนัข
          </div>
          <div className="btn rounded-full bg-slate-200">
            <span className="text-xl">🫎</span>สัตว์ประเภทอื่นๆ
          </div>
        </div>
        <div className="hidden lg:flex gap-3">
          <button
            className="btn bg-pethub-color6 rounded-full hover:bg-pethub-color1 text-white"
      ...
      <div
        ref={parentRef}
        className="mt-10 w-11/12 xl:w-8/12 overflow-hidden mx-auto relative h-[83vw] md:h-[590px]"
      >
        <motion.div
          ref={containerRef}
          className="absolute h-[80vw] md:h-[570px] rounded-md mx-auto p-5 flex gap-5"
          drag="x"
