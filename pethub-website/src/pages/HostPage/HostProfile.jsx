...
function HostProfile() {
  ...
  const updateHotelProfile = async () => {
    try {
      ...
      const response = await axios.put(
        `https://pethub-3nkk.onrender.com/api/getHotelProfile/updateHotelProfile/${fName}/${lName}`,
        updatedData,
  ...
  const handleDetailClick = (roomID) => {
    ...
    naigate(`./pethub-website/hostprofile/${roomID}`);
  };
    ...
    const fetchHotelProfile = async () => {
      try {
        ...
        const response = await axios.get(
          `https://pethub-3nkk.onrender.com/api/getHotelProfile/hostprofile/${fName}/${lName}`,
          { withCredentials: true }
        );
        console.log(
          `URL: https://pethub-3nkk.onrender.com/api/getHotelProfile/hostprofile/${fName}/${lName}`
        );
  ...
  return (
    <>
      <Navbar />
      <div className=" text-[4.5vw] md:text-3xl lg:text-4xl text-start mt-24 md:mt-28 lg:mt-32 w-11/12 xl:w-10/12 max-w-[1200px] mx-auto font-bold text-pethub-color1">
        PETHUB <span className="font-normal text-black">Hotel Management</span>
      </div>
        ...
        <div className="h-full flex flex-col justify-between grow px-[2vw] pt-[2vw] md:px-4 md-pt-4 lg:px-7 lg:pt-7">
          <div className="flex flex-col gap-[1vw] md:gap-3">
            <div className="flex justify-between items-end">
              <div className="text-[2.5vw] md:text-lg lg:text-2xl font-semibold">
                ชื่อจดทะเบียน:{" "}
                <span className="text-pethub-color1">{hotelName}</span>
              </div>
              <div className="hidden md:block text-[1.5vw] md:text-xs lg:text-lg">
                {hotelID}
              </div>
            </div>
            <div className="text-[2vw] md:text-sm lg:text-lg font-semibold transition-all duration-300 ease-in-out line-clamp-1 overflow-hidden">
              สถานที่ตั้ง:{" "}
              <span className="text-pethub-color6 font-normal">
                {hotelAddress}
              </span>
            </div>
            <div className="text-[2vw] md:text-sm lg:text-lg font-semibold">
              ประเภทที่พัก:{" "}
              <span className="text-pethub-color6 font-normal">
                {hotelType}
              </span>
          ...
          <div className="flex justify-start gap-5 md:gap-10 items-center">
            <div className="text-[2.5vw] md:text-base lg:text-xl font-semibold">
              ผู้ดูแล:{" "}
              <span className="text-[2vw] md:text-sm lg:text-base text-pethub-color6 font-normal">
                {fName} {lName}
              </span>
            </div>
            <div className="text-[2.5vw] md:text-base lg:text-xl font-semibold">
      ...
      <div className="lg:px-14 text-[4vw] md:text-2xl lg:text-3xl text-start mt-[6vw] md:mt-10 lg:mt-16 w-11/12 xl:w-10/12 max-w-[1200px] mx-auto font-semibold">
        รายละเอียดโรงแรม
      </div>
      <div className="lg:px-20 my-2 md:my-5 w-11/12 xl:w-10/12 max-w-[1200px] mx-auto">
        ...
        <button
          ...
          } flex justify-center items-center rounded-md md:btn  text-white md:text-white h-[7vw] w-[15vw] sm:w-24 sm:h-10 md:w-28 font-medium text-[2vw] md:text-xs lg:text-sm xl:text-base`}
            ...
            {rooms?.map((room, index) => (
              ...
              >
                ...
                <div className="w-full h-[50%] p-[2vw] md:p-6 text-start">
                  <div className="flex justify-between items-end">
                    <div className="text-[2.5vw] md:text-lg lg:text-xl xl:text-2xl">
                      {room.roomTypeName}
                    </div>
                    <div className="text-[2vw] md:text-sm lg:text-base xl:text-lg">
                      {room.roomSize} ตร.ม.
                    </div>
                  </div>
                  {/* <div className='text-[1.8vw] md:text-sm lg:text-sm xl:text-lg md:my-1 lg:my-3'>ของเล่นแมว, อาหาร, อาบน้ำ, ดูแล 24 ชั่วโมง</div> */}
                  <div className="text-[1.8vw] md:text-sm lg:text-sm xl:text-lg md:my-1 lg:my-3">
                    {room.petAllowedType}
                  </div>
                  <div className="text-[1.5vw] md:text-xs lg:text-sm xl:text-base transition-all duration-300 ease-in-out line-clamp-2 overflow-hidden text-gray-400">
                    {room.roomDetail}
                  </div>
                  <div className="text-[2vw] md:text-base my-[1vw] md:my-2 lg:my-5">
                    {room.pricePerNight} บาท/คืน
                  </div>
                  <button
                ...
                <tbody key={booking.bookingID}>
                  <tr className="text-xs lg:text-sm xl:text-base">
                    ...
                    <td className="max-md:pr-0">
                      <span className="badge badge-ghost badge-sm bg-green-100 text-[2.5vw] sm:text-xs lg:text-sm xl:text-base">
                    ...
                    <th>
                      <button
                        ...
                        onClick={() => handleDetailClick(booking.roomTypeID)}
                      >
