...
function HotelDetail() {
  ...
  const emptyStarIconURL =
    "M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"; // Example
  const filledStarIconURL =
  ...
  const goBooking = (room) => {
    ...
    if (!token) {
      ...
      if (role == "Client" || role == "Dev") {
        ...
        naigate(
          `./pethub-website/home/${hotelData.hotelName}/${room.roomTypeName}`,
          { state: roomData, hotelState: hotelData }
        );
      } else {
    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          `https://pethub-3nkk.onrender.com/api/review/getReview/${hotelData.hotelID}`,
  ...
  useEffect(() => {
    ...
    const loadScript = () => {
      ...
      script.src =
        "https://api.longdo.com/map/?key=2b0da15d101163c9f1968c6c656c8ca5";
      script.async = true;
    ...
    const initMap = () => {
      ...
      var marker = new window.longdo.Marker({
        lon: hotelData.mapLong,
        lat: hotelData.mapLat,
      });
      ...
      <div className="mt-5 md:mt-10 w-[96vw] lg:w-11/12 xl:w-11/12 2xl:w-[1280px] h-full mx-auto">
        <div className="w-full lg:h-20 flex gap-5 text-[10px] sm:text-[2vw] md:text-lg xl:text-xl font items-center">
          <div
            ...
            className={`px-3 lg:px-5 py-1 lg:py-2 border-pethub-color1 cursor-pointer ${
              barSel == 0 ? " border-b-2 font-bold" : ""
            }`}
          >
            Overview
          </div>
      ...
      <div className="w-full h-full bg-[#F4F4F4]">
        <div className="w-[90vw] lg:w-11/12 xl:w-10/12 2xl:w-[1280px] h-full md:h-[600px] lg:h-[1000px] mx-auto py-[1vw] lg:py-10 flex flex-wrap md:grid grid-cols-12 grid-rows-12 gap-x-[3%] md:gap-4">
          <div className="max-md:basis-full col-span-12 md:col-span-8 row-span-3 lg:row-span-2 max-md:py-[2vw] max-lg:py-5">
            <h1 className="text-start text-[5vw] md:text-2xl lg:text-3xl xl:text-5xl uppercase font-bold ">
              {hotelData.hotelName}
            </h1>
            <div className="flex my-[1vw] md:my-3 lg:my-4 gap-1 items-center">
              {Array.from({ length: hotelData.avgReviewScore }).map(
                (_, index) => (
                  <svg
                    key={index}
                    ...
                    height="16"
                    fill="currentColor"
                    className="bi bi-star-fill w-[8px] sm:w-[12px] lg:w-[16px] text-yellow-400"
                    viewBox="0 0 16 16"
                  >
              ...
              <div className="max-md:text-[2vw] ml-1 md:ml-3">
                {parseFloat(hotelData.avgReviewScore).toFixed(2)} (
                {hotelData.reviewCount} Reviews)
              </div>
            </div>
            <a
              href={`https://www.google.com/maps?q=${hotelData.mapLat},${hotelData.mapLong}`}
              target="_blank"
              className="flex gap-3 items-center justify-start text-gray-400"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                height="24"
                fill="currentColor"
                className="cursor-pointer bi bi-geo-alt text-blue-400 max-md:w-[16px]"
                viewBox="0 0 16 16"
              >
                <path d="M12.166 8.94c-.524 1.062-1.234 2.12-1.96 3.07A32 32 0 0 1 8 14.58a32 32 0 0 1-2.206-2.57c-.726-.95-1.436-2.008-1.96-3.07C3.304 7.867 3 6.862 3 6a5 5 0 0 1 10 0c0 .862-.305 1.867-.834 2.94M8 16s6-5.686 6-10A6 6 0 0 0 2 6c0 4.314 6 10 6 10" />
                <path d="M8 8a2 2 0 1 1 0-4 2 2 0 0 1 0 4m0 1a3 3 0 1 0 0-6 3 3 0 0 0 0 6" />
              </svg>
              <div className="hover:text-blue-500 cursor-pointer text-start text-[10px] sm:text-[2vw] md:text-[14px] xl:text-lg  transition-all duration-300 ease-in-out max-lg:line-clamp-1 max-lg:overflow-hidden">
          ...
          <div
            className={`max-md:w-[60%] ${
              barSel == 1 ? "max-md:h-full" : "max-md:h-[250px]"
            }   overflow-y-hidden bg-white rounded-md col-span-8 lg:col-span-8 row-span-10 max-md:p-[3vw] max-lg:px-8 max-lg:py-4 py-12 px-14 text-start`}
          >
            <div
              className={`text-[3vw] lg:text-2xl font-semibold mb-[1vw] md:mb-6 overflow-y  ${
                barSel == 0 ? "" : "hidden"
              }`}
            >
              Overview
            ...
            <div
              className={`text-[2vw] md:text-sm max-md:h-[160px] h-[80%] lg:h-[90%] px-4 overflow-y-scroll hide-scrollbar ${
                barSel == 0 ? "" : "hidden"
              }`}
            >
              <p>{hotelData.hotelDescription}</p>
              <div className="text-md lg:text-xl my-[1vw] md:my-6 overflow-y">
                ข้อกำหนด
              </div>
              <div>
            ...
            <div
              className={`text-[2vw] lg:text-xl font-semibold mb-[1vw] md:mb-6 overflow-y  ${
                barSel == 1 ? "" : "hidden"
              }`}
            >
              {parseFloat(hotelData.avgReviewScore).toFixed(2)}{" "}
              <span className="text-[1.5vw] lg:text-lg">
                {"รีวิวจากผู้ใช้ " + hotelData.reviewCount + " รายการ"}
              </span>
            </div>
            <div
              className={`mb-[2vw] md:mb-5 text-[1.5vw] md:text-xs lg:text-sm xl:text-base text-gray-400 ${
                barSel == 1 ? "" : "hidden"
              }`}
            ></div>
            <div
              className={`text-[2vw] md:text-sm max-md:h-[160px] h-[45%] lg:h-[60%] md:px-2 xl:px-4 overflow-y-scroll scrollbar-hidden flex flex-col gap-1 lg:gap-5 ${
                barSel == 1 ? "" : "hidden"
              }`}
            >
              {reviewData ? (
                reviewData.map((review, index) => (
                  <div
                    key={index}
                    className="w-full h-[10vw] md:h-20 lg:h-28 flex"
                  >
                    <div className="h-full w-[80%] md:py-2 px-1 lg:px-5 text-[1.5vw] md:text-xs lg:text-base">
                      ...
                      <div className="flex md:my-[1vw] lg:my-1 items-center">
                        {Array.from({ length: review.reviewScore }).map(
                          (_, index) => (
                            <svg
                              key={index}
                              xmlns="http://www.w3.org/2000/svg"
                              ...
                              height="12"
                              fill="currentColor"
                              className="bi bi-star-fill w-[1.5vw] md:w-[8px] mx-[0.2vw] lg:mx-0 sm:w-[10px] lg:w-[16px] text-yellow-400"
                              viewBox="0 0 16 16"
                            >
                              <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z" />
                            </svg>
              ...
              <div className="w-full h-[18vw] md:h-[160px] lg:h-[200px] flex py-[3vw] md:py-5 lg:py-8">
                ...
                <div className="h-full w-[20%] md:w-[30%] xl:w-[25%] cursor-pointer flex items-center md:items-end py-[1vw] md:py-7 justify-between gap-1 text-[2vw] md:text-xs lg:text-base">
                  ...
                  <svg
                    ...
                    height="16"
                    fill="currentColor"
                    className="md:hidden w-[2vw] h-[2vw] text-base-300 bi bi-send-fill"
                    viewBox="0 0 16 16"
                  >
          ...
          <div className="max-md:w-[35%] max-md:h-full bg-white rounded-md col-start-9 col-span-4 row-span-5 p-[3vw] md:p-3 lg:p-5">
            <div className="flex gap-[1vw] lg:gap-2 items-center justify-center">
              <div className="text-[2vw] lg:text-xl font-semibold transition-all duration-300 ease-in-out line-clamp-1 overflow-hidden">
                โรงแรม {param.hotelname}
              </div>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="w-[3vw] lg:w-12"
                width={20}
                height={20}
                viewBox="0 0 640 512"
              >
            ...
            <div className="flex flex-col justify-between h-[90%] text-[2vw] md:text-sm lg:text-lg">
              <div className="w-full ">
                <div className="flex justify-between items-center mt-[1vw] md:mt-5">
                  ...
                  <div className="max-w-60 font-normal transition-all duration-300 ease-in-out line-clamp-1 overflow-hidden">
                    {hotelData.hotelType}
                  </div>
                </div>
                <div className="flex justify-between items-start mt-2"></div>
                <div className="flex justify-between items-start mt-2">
                  <div className="text-start transition-all duration-300 ease-in-out line-clamp-1 overflow-hidden">
                    สัตว์เลี้ยงที่รับ:{" "}
                  </div>
                ...
                <div className="flex justify-between items-start mt-2">
                  ...
                  <div className="flex justify-end gap-2">
                    <div className="max-w-40 font-normal transition-all duration-300 ease-in-out line-clamp-1 overflow-hidden">
                      {" "}
                      {hotelData.roomsAvailable[0].pricePerNight} บาท
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="mt-5 md:mt-5 w-[96vw] lg:w-11/12 xl:w-11/12 2xl:w-[1280px] h-full mx-auto px-5">
          <div className="flex items-start lg:items-center justify-between flex-col lg:flex-row">
            <div className="text-start text-[5vw] sm:text-xl lg:text-2xl xl:text-3xl font-semibold">
              Available rooms
            </div>
          </div>
          <div
            ref={parentRef}
            className="md:mt-4 lg:mt-8 xl:mt-10 w-full overflow-hidden mx-auto relative h-[70vw] md:h-[550px]  lg:h-[650px]"
          >
            <motion.div
              ref={containerRef}
              className="absolute h-[60vw] md:h-[500px] lg:h-[600px] rounded-md mx-auto md:py-5 flex gap-3 lg:gap-5"
              drag="x"
            ...
            >
              {hotelData.roomsAvailable.map((room, index) => (
                <div
                  key={index}
                  className="w-[45vw] md:w-[350px] xl:w-[400px] bg-white rounded-lg overflow-hidden"
                >
                  ...
                  <div className="w-full h-[50%] p-[2vw] md:p-6 text-start">
                    <div className="flex justify-between items-end">
                      <div className="text-[2.5vw] md:text-lg lg:text-xl xl:text-2xl transition-all duration-300 ease-in-out line-clamp-1 overflow-hidden">
                        {room.roomTypeName} ({room.petAllowedType})
                      </div>
                      <div className="text-[2vw] md:text-sm lg:text-base xl:text-lg transition-all duration-300 ease-in-out line-clamp-1 overflow-hidden">
                        {room.roomSize} ตรม
                      </div>
                    </div>
                    <div className="text-[2vw] md:text-xs lg:text-sm xl:text-base transition-all duration-300 ease-in-out line-clamp-2 overflow-hidden text-gray-400 ">
                      {room.roomDetail.length > 150
                        ? room.roomDetail.substring(0, 150) + "..."
                        : room.roomDetail}
                    </div>
                    <div className="text-[2vw] md:text-base my-[1vw] md:my-2 lg:my-5">
                      {parseFloat(room.pricePerNight).toFixed(0)} บาท/คืน
                    </div>
                    <button
                      onClick={() => goBooking(room)}
